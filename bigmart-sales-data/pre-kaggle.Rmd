...
# train_y_mm <- train_x_mm$Item_Outlet_Sales
```
...
# cvlasso_pred <- predict(store_lasso, s=best_lam, newx=test_x)

```

To do:

1. Redo the model_mat
    a)  train and test as row numbers
    b) x as model matrix with identifier
    c) y as sales_output
